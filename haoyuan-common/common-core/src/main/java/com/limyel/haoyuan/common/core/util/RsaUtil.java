...
public class RsaUtil {
    ...
    public static PublicKey getPublicKey(String filename) {
        try {
            ...
            byte[] publiKeyBytes = FileCopyUtils.copyToByteArray(resource.getInputStream());
            byte[] decode = Base64Utils.decode(publiKeyBytes);
    ...
    public static void generateKey(String publicKeyFilename, String privateKeyFilename, String secret) {
        try {
            ...
            publicKeyBytes = Base64Utils.encode(publicKeyBytes);
            writeFile(publicKeyFilename, publicKeyBytes);

            // 获取私钥并写出
            byte[] privateKeyBytes = keyPair.getPrivate().getEncoded();
            privateKeyBytes = Base64Utils.encode(privateKeyBytes);
